...
for (const user of users) {
  ...
  for (const sound in user.favoritesSounds) {
    //Recorrer cada sonido favorito.
    if (totalFavorites[sound]) {
      totalFavorites[sound]++;
    } else {
      totalFavorites[sound] = 1;
    }
  }
}

for (const sound in totalFavorites) {
  console.log(`${sound}: ${totalFavorites[sound]} veces`);
}
